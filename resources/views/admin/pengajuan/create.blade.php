...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        // Function untuk menampilkan/menyembunyikan input jumlah
        function toggleInput(asetId) {
            ...
            if (checkbox.checked) {
                ...
                jumlahInput.removeAttribute("disabled"); // Aktifkan input jumlah
            } else {
                jumlahInput.style.display = "none";
                jumlahInput.setAttribute("disabled", "true"); // Nonaktifkan agar tidak dikirim
            }
        }

        // Event listener untuk setiap checkbox aset
        document.querySelectorAll('.aset-checkbox').forEach(checkbox => {
            checkbox.addEventListener('change', function () {
                let asetId = this.getAttribute('data-id');
                toggleInput(asetId);
            });
        });

        // Pastikan input jumlah yang tidak dipilih tidak terkirim
